<x-app-layout>
                                    ...
                                    <td class="w-4 p-4">
                                        <div class="flex items-center">
                                            <input class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 dark:focus:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600"
                                                   ...
                                                   v-on:change="e => selectItem(role, e.target.checked)">
                                            <label :for="'role-' + role.id" class="sr-only">Select</label>
                                        </div>
                                    </td>
                                    <td scope="row" class="flex items-center px-6 py-4 text-gray-900 whitespace-nowrap dark:text-white">
                                        <div class="text-base font-semibold">
                                            <a class="font-medium text-blue underline dark:text-blue-500 hover:no-underline hover:text-gray"
                                               :href="'{{ route('settings.roles.index') }}/' + role.id + '/edit'" v-text="role.name"></a>
                                        </div>
                                    </td>
    ...
    <form name="deleteForm" action="{{ route('settings.roles.destroy') }}" method="POST">
        @csrf
        @method('DELETE')
        <input type="hidden" name="roles" v-model="selectedItems">
    </form>
            ...
            createApp({
                setup() {
                    ...
                    const selectAll = (checked) => {
                        if (checked) selectedItems.value = roles.value.map(item => item.id);
